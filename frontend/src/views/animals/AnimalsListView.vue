...
import {
  ...
  listSpecies, listBreeds,
  listWeightHistories, createWeightHistory, deleteWeightHistory,
} from '../../api'
...
function openCreate() {
  ...
  form.value = {
    tag_no: '', name: '', species_id: null, breed_id: null,
    gender: 'Male', birth_date: '', purchase_date: '', purchase_price: 0,
    color: '', status: 'Active', remarks: '',
  }
  ...
  <div>
    <div class="mb-6 flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-gray-500">Manage livestock records</p>
      </div>
    ...
    <Modal :show="showModal" :title="editingId ? 'Edit Animal' : 'Add Animal'" @close="showModal = false" size="lg">
      <form @submit.prevent="save">
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Tag No</label>
            <input v-model="form.tag_no" type="text" required class="rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm focus:border-brand-400 focus:ring-2 focus:ring-brand-500/20 outline-none transition-all w-full" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Name</label>
            <input v-model="form.name" type="text" class="rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm focus:border-brand-400 focus:ring-2 focus:ring-brand-500/20 outline-none transition-all w-full" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Species</label>
            <select v-model="form.species_id" required @change="form.breed_id = null" class="rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm focus:border-brand-400 focus:ring-2 focus:ring-brand-500/20 outline-none transition-all w-full">
          ...
          <div>
            ...
            <select v-model="form.breed_id" class="rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm focus:border-brand-400 focus:ring-2 focus:ring-brand-500/20 outline-none transition-all w-full">
          ...
          <div>
            ...
            <select v-model="form.gender" required class="rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm focus:border-brand-400 focus:ring-2 focus:ring-brand-500/20 outline-none transition-all w-full">
          ...
          <div>
            ...
            <input v-model="form.birth_date" type="date" class="rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm focus:border-brand-400 focus:ring-2 focus:ring-brand-500/20 outline-none transition-all w-full" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Purchase Date</label>
            <input v-model="form.purchase_date" type="date" class="rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm focus:border-brand-400 focus:ring-2 focus:ring-brand-500/20 outline-none transition-all w-full" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Purchase Price</label>
            <input v-model="form.purchase_price" type="number" min="0" step="0.01" class="rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm focus:border-brand-400 focus:ring-2 focus:ring-brand-500/20 outline-none transition-all w-full" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Color</label>
            <input v-model="form.color" type="text" class="rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm focus:border-brand-400 focus:ring-2 focus:ring-brand-500/20 outline-none transition-all w-full" />
          </div>
        </div>
        <div class="mt-4">
        ...
        <div v-if="editingId" class="border-t pt-4 mt-4">
          <div class="mb-3 flex items-center justify-between">
            ...
            <button type="button" @click="showWeightForm = !showWeightForm" class="text-sm font-medium text-brand-600 hover:text-brand-700">{{ showWeightForm ? 'Cancel' : 'Add Weight' }}</button>
          </div>
          ...
          <div v-if="showWeightForm" class="mb-3 flex items-end gap-3">
            <div class="flex-1">
              ...
              <input v-model="weightForm.weight" type="number" step="0.01" required class="rounded-xl border border-gray-200 px-3.5 py-2.5 text-sm focus:border-brand-400 focus:ring-2 focus:ring-brand-500/20 outline-none transition-all w-full" />
            </div>
            <div class="flex-1">
            ...
            <button type="button" :disabled="weightSaving" @click="addWeight" class="px-4 py-2.5 text-sm font-medium text-white bg-brand-600 rounded-xl hover:bg-brand-700 disabled:opacity-50">{{ weightSaving ? '...' : 'Save' }}</button>
          </div>
          ...
          <div class="overflow-x-auto rounded-xl border border-gray-100">
            <table class="w-full text-left text-sm">
              <thead class="border-b bg-gray-50">
                <tr>
                  <th class="px-3 py-2 font-medium text-gray-500 text-xs uppercase">Weight</th>
                  <th class="px-3 py-2 font-medium text-gray-500 text-xs uppercase">Date</th>
                  <th class="px-3 py-2 font-medium text-gray-500 text-xs uppercase">Remarks</th>
                  <th class="px-3 py-2 text-right">Action</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-gray-50">
                <tr v-if="weightHistoriesForAnimal.length === 0">
                  <td colspan="4" class="px-3 py-4 text-center text-gray-400">No weight records.</td>
                </tr>
                <tr v-for="w in weightHistoriesForAnimal" :key="w.id">
                  <td class="px-3 py-2">{{ w.weight }} kg</td>
                  <td class="px-3 py-2">{{ w.record_date ? new Date(w.record_date).toLocaleDateString() : '-' }}</td>
                  <td class="px-3 py-2">{{ w.remarks || '-' }}</td>
                  <td class="px-3 py-2 text-right">
                    <button type="button" @click="removeWeight(w.id)" class="text-xs font-medium text-red-500 hover:text-red-600">Delete</button>
                  </td>
                </tr>
              </tbody>
            </table>
